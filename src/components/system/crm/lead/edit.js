...
import { GetCrmLeadById, PutCrmLead } from "..//shared/lead";
...
let EditCrmLead = props => {
  // getModalStyle is not a pure function, we roll the style only on the first render

  async function onSubmit(values, { setSubmitting, setErrors }) {
    if (!(values.expectedSaleTime === null)) {
    ...
    if (values.leadDate === null) {
      delete values.leadDate;
    }
    if (values.expectedSaleTime === null) {
      delete values.expectedSaleTime;
    }
    delete values.expectedSaleTimemodified;
    console.log(values);
    await PutCrmLead(props.IDforAPI, values)
  ...
  async function getlistapi() {
    const { data: initialValues } = await GetCrmLeadById(props.IDforAPI);
    setInitialValues(initialValues);

    if (!(initialValues.expectedSaleTime === null)) {
      initialValues.expectedSaleTimemodified = initialValues.expectedSaleTime.substr(
        11,
        initialValues.expectedSaleTime.length - 4
      );

      initialValues.expectedSaleTime = initialValues.expectedSaleTime.substr(
        0,
        initialValues.expectedSaleTime.length - 9
      );
    }
    if (!(initialValues.leadDate === null)) {
      initialValues.leadDate = initialValues.leadDate.substr(
        0,
        initialValues.leadDate.length - 9
      );
    }

    if (
      !(initialValues.status === null || initialValues.status === undefined)
    ) {
      initialValues.status = initialValues.status.trim();
    }

    if (
      !(initialValues.source === null || initialValues.source === undefined)
    ) {
      initialValues.source = initialValues.source.trim();
    }
    if (
      !(initialValues.priority === null || initialValues.priority === undefined)
    ) {
      initialValues.priority = initialValues.priority.trim();
    }
    if (
      !(
        initialValues.considered === null ||
        initialValues.considered === undefined
      )
    ) {
      initialValues.considered = initialValues.considered.trim();
    }
                              ...
                              <div className="row">
                                ...
                                <div className="col-12 col-sm-12 col-md-6 col-lg-9 col-xl-9">
                                  ...
                                  <Input
                                    ...
                                    onBlur={handleBlur}
                                    value={values.new}
                                    defaultChecked={values.new ? true : false}
                                  />
                                  &nbsp;&nbsp;&nbsp;
                                  <Label for="new" style={classes.label}>
                                    New
                              ...
                              <div className="row">
                                ...
                                <div className="col-12 col-sm-12 col-md-6 col-lg-9 col-xl-9">
                                  ...
                                  <Input
                                    ...
                                    defaultChecked={
                                      values.existingContact ? true : false
                                    }
                                  />
                                  &nbsp;&nbsp;&nbsp;
                                  <Label
