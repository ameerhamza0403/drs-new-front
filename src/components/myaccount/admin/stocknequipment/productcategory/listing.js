...
const options = {
  ...
  async function getlistapi() {
    await GetListingForProductCategory(Page, PageSize).then(res => {
...
//----- Finished Pagination---------
  ...
  async function Dellistapi() {
    await DeleteProductCategoryDataById(idofEdit).then(() => {
      success();
    }).catch(error => {
        errort();
      });
  ...
  function HandlerowSelect  (row) {
    menuDiv = "";
    idofEdit = row.productCategoryId;
  ...
  if (menushow) {
    menuDiv = (
      <ul className="tool">
        <li>
          <AddProductCategory refresh={refreshfn} />
        </li>
    ...
    <div>
      <div className="row header">
        ...
        <div className="col-12 col-sm-6 col-md-7 col-lg-7 col-xl-7">
          <h3 className="heading">PRODUCT CATEGORY</h3>
        </div>
      </div>
      <br />
      {EditshowModel}
