import { GetTemplateKeywordDataById, PutTemplateKeywordDataById } from "../shared/templatekeywords";
...
let EditTemplateKeyword = props => {
  ...
  async function onSubmit(initialValues, { setSubmitting, setErrors }) {
    await PutTemplateKeywordDataById(props.IDforAPI, initialValues).then(()=>success()).catch(error=>errort());
    handleOpen();
    props.refresh();
    setSubmitting(false);
  }
...
//   });
  async function getlistapi() {
    const { data: initialValues } = await GetTemplateKeywordDataById(props.IDforAPI);
...
export default EditTemplateKeyword;
