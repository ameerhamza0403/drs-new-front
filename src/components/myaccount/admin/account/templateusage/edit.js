import { GetTemplateUsageDataById, PutTemplateUsageDataById } from "../shared/templateusage";
...
let EditTemplateUsage = props => {
  ...
  async function onSubmit(initialValues, { setSubmitting, setErrors }) {
    await PutTemplateUsageDataById(props.IDforAPI, initialValues).then(()=>success()).catch(error=>errort());
    handleOpen();
    props.refresh();
    setSubmitting(false);
  }
...
//   });
  async function getlistapi() {
    const { data: initialValues } = await GetTemplateUsageDataById(props.IDforAPI);
                        ...
                        <div className="row">
                          ...
                          <div className="col-12 col-sm-12 col-md-6 col-lg-8 col-xl-8 mb-3">
                            ...
                            >
                               <option selected />
                                  {usagetypedata.map(e => (
                                    <option value={e.templateUsageTypeId }>
                                      {e.name}
                                    </option>
                                  ))}
                            </Input>
...
export default EditTemplateUsage;
