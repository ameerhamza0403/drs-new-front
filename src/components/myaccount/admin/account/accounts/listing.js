...
const options = {
  ...
  async function getlistapi() {
    await GetListingForAccount(Page, PageSize).then(res => {
  ...
  if (Tabledistatus) {
    Tabledisplay = (
        ...
        >
          ...
          <TableHeaderColumn dataField="addressLine1" dataSort>
            Address Line 1
          </TableHeaderColumn>
...
//----- Finished Pagination---------
  ...
  async function Dellistapi() {
    await DeleteAccountDataById(idofEdit).then(() => {
      success();
    }).catch(error => {
        errort();
      });
