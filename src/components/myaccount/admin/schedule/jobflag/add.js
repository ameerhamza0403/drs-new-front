...
let AddJobFlag = props => {
  ...
  const handleChange = name => event => {
    if (name != "isActive") {
      ...
      setInitialValues({ ...initialValues, [name]: event.target.checked });
    }
  };
  ...
  // async function postlistapi(values) {
  ...
  async function onSubmit(values, { setSubmitting, setErrors }) {
    ...
    console.log(values);
    await PostListingForJobflag(values).then(()=>success()).catch(error=>errort());
    handleOpen();
    props.refresh();
    setSubmitting(false);
  }
  ...
  return (
    <div>
      ...
      <Modal
        ...
        size="lg"
      >
        <ModalHeader toggle={handleOpen} ><h3 className="font-weight:bold;">Add Job Flag</h3></ModalHeader>
