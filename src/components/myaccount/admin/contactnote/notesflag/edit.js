import { GetNoteflagDataById, PutNoteflagDataById } from "../shared/notesflag";
...
let EditButtonForNotesFlag = props => {
  let ColorCode = [
    "FFFFE0",
    "FFFACD",
    "FFEFD5",
  ...
  async function onSubmit(values, { setSubmitting, setErrors }) {
    ...
    values.colorCode = valofCod;
    await PutNoteflagDataById(props.IDforAPI, values).then(()=>success()).catch(error=>errort());
    handleOpen();
    props.refresh();
    setSubmitting(false);
  }
  ...
  async function getlistapi() {
    const { data: initialValues } = await GetNoteflagDataById(props.IDforAPI);
    
    setInitialValues(initialValues);
    valofCod = initialValues.colorCode;
    setModal(true);
  }
  ...
  return (
    <div>
      ...
      <Modal
        ...
        size="lg"
      >
        <ModalHeader toggle={handleOpen} ><h3 className="font-weight:bold;">Edit Person Flag</h3></ModalHeader>
...
export default EditButtonForNotesFlag;
