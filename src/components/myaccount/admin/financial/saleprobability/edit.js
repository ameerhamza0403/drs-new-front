...
let EditButtonforSaleOppProb = props => {
  ...
  async function getlistapi() {
    const { data: editvalues } = await GetSaleProbDataById(props.IDforAPI);
    seteditValues(editvalues);
  }

  async function putlistapi() {
    console.log(editvalues);
    await PutSaleProbDataById(props.IDforAPI, editvalues).then(res => success(res.data.message)).catch(error=>errort());
    handleOpen();
    props.refresh();
  }
