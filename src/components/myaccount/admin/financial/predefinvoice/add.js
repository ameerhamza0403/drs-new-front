import {
  PostListingForPredefInvoiceItem,
...
let PredefInvAdd = props => {
  ...
  async function onSubmit(values, { setSubmitting, setErrors }) {
    ...
    values.productCategoryId = parseInt(values.productCategoryId);
    values.sellingPrice = parseFloat(values.sellingPrice);
    values.costPrice = parseFloat(values.costPrice);
    values.taxCodeId = parseInt(values.taxCodeId);
    delete values.predefinedItemId;
    console.log(values);
    await PostListingForPredefInvoiceItem(values)
      .then(() => success())
      .catch(error => errort());
    handleOpen();
    props.refresh();
                        ...
                        <div className="row mb-2">
                          ...
                          <div className="col-12 col-sm-12 col-md-6 col-lg-8 col-xl-8">
                            ...
                            >
                              <option selected></option>
                              {currency.map(e => (
                                <option value={e.currencyId}>{e.name}</option>
                              ))}
                            </Input>
