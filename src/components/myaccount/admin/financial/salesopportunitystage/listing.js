...
import EditSalesOpportunityStage from "./edit";
import {
  GetListingForSalesOpportunityStage,
  DeleteSalesOpportunityStageDataById
} from "..//shared/salesopportunitystage";
import AddSalesOpportunityStage from "./add";
...
const classes = {
  ...
  let SalesOpportunityStageListing = () => {
...
const options = {
  ...
  async function getlistapi() {
    await GetListingForSalesOpportunityStage(Page, PageSize).then(res => {
    ...
    Atlist.map(
      (e, i) =>
        (Atlist[i].color = (
          <div className="ColorCodesl" style={ColorStyleFn(e.color)} />
        ))
    );
  ...
  if (Tabledistatus) {
    Tabledisplay = (
        ...
        >
          ...
          <TableHeaderColumn dataField="name" hidden={true} isKey={true} dataSort>
            Name
          </TableHeaderColumn>
...
//----- Finished Pagination---------
  ...
  async function Dellistapi() {
    await DeleteSalesOpportunityStageDataById(idofEdit).then(() => {
      success();
    }).catch(error => {
        errort();
      });
  ...
  function HandlerowSelect  (row) {
    menuDiv = "";
    idofEdit = row.salesOpportunityStageId;
  ...
  if (menushow) {
    menuDiv = (
      <ul className="tool">
        <li>
          <AddSalesOpportunityStage refresh={refreshfn} />
        </li>
    ...
    <div>
      <div className="row header">
        ...
        <div className="col-12 col-sm-6 col-md-7 col-lg-7 col-xl-7">
          <h3 className="heading">SALES OPPORTUNITY STAGE</h3>
        </div>
      </div>
      <br />
      {EditshowModel}
...
export default SalesOpportunityStageListing;
